...
var externalJavascriptFiles = [
    ...
    'plugins/vendor/inputmask/js/jquery.inputmask.bundle.js'
]

console.log(externalJavascriptFiles.length + " number of file(s).");

fs.writeFile("vendors-form-elements.js", "", (err) => {
    if (err) console.log(err);
    console.log("Emptied vendors-form-elements.js file.");
});
...
externalJavascriptFiles.map((element, index) => {
    fs.readFile(element, "utf-8", (err, data) => {
        if (err) { console.log(err) }
        setTimeout(() => {
            fs.appendFile('vendors-form-elements.js', data.toString() + os.EOL, function (err) {
