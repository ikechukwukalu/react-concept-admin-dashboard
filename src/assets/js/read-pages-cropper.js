...
var externalJavascriptFiles = [
    ...
    'plugins/vendor/cropper/dist/cropper.min.js',
    'plugins/vendor/cropper/dist/cropper-int.js'
]

console.log(externalJavascriptFiles.length + " number of file(s).");

fs.writeFile("vendors-pages-cropper.js", "", (err) => {
    if (err) console.log(err);
    console.log("Emptied vendors-pages-cropper.js file.");
});
...
externalJavascriptFiles.map((element, index) => {
    fs.readFile(element, "utf-8", (err, data) => {
        if (err) { console.log(err) }
        setTimeout(() => {
            fs.appendFile('vendors-pages-cropper.js', data.toString() + os.EOL, function (err) {
