...
var externalJavascriptFiles = [
    ...
    'plugins/vendor/full-calendar/js/moment.min.js',
    'plugins/vendor/full-calendar/js/fullcalendar.js',
    'plugins/vendor/full-calendar/js/jquery-ui.min.js',
    'plugins/vendor/full-calendar/js/calendar.js',
    'plugins/libs/js/main-js.js'
]

console.log(externalJavascriptFiles.length + " number of file(s).");

fs.writeFile("vendors-pages-calender.js", "", (err) => {
    if (err) console.log(err);
    console.log("Emptied vendors-pages-calender.js file.");
});
...
externalJavascriptFiles.map((element, index) => {
    fs.readFile(element, "utf-8", (err, data) => {
        if (err) { console.log(err) }
        setTimeout(() => {
            fs.appendFile('vendors-pages-calender.js', data.toString() + os.EOL, function (err) {
