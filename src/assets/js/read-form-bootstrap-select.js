...
var externalJavascriptFiles = [
    ...
    'plugins/vendor/bootstrap-select/js/bootstrap-select.js'
]

console.log(externalJavascriptFiles.length + " number of file(s).");

fs.writeFile("vendors-form-bootstrap-select.js", "", (err) => {
    if (err) console.log(err);
    console.log("Emptied vendors-form-bootstrap-select.js file.");
});
...
externalJavascriptFiles.map((element, index) => {
    fs.readFile(element, "utf-8", (err, data) => {
        if (err) { console.log(err) }
        setTimeout(() => {
            fs.appendFile('vendors-form-bootstrap-select.js', data.toString() + os.EOL, function (err) {
