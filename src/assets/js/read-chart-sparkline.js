...
fs.writeFile("vendors-chart-sparkline.js", "", (err) => {
    if (err) console.log(err);
    console.log("Emptied vendors-chart-sparkline.js file.");
});
...
externalJavascriptFiles.map((element, index) => {
    fs.readFile(element, "utf-8", (err, data) => {
        if (err) { console.log(err) }
        setTimeout(() => {
            fs.appendFile('vendors-chart-sparkline.js', data.toString() + os.EOL, function (err) {
